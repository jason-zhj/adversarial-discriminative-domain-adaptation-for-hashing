...
def train(enc, dcd, clf, src_loader, tgt_loader, params):
    ...
    loss_records = {"dcd_confusion":[],"clf":[],"dcd_clf":[]}
    ...
    for i in range(params.iterations):
        ...
        for step, ((images_src, labels_src), (images_tgt, labels_tgt)) in loader:
            ...
            clf_tgt_out = clf(g_tgt)
            clf_loss = compute_classification_loss(outputs=torch.cat([clf_src_out,clf_tgt_out]),labels=torch.cat([labels_src,labels_tgt]))
            dcd_confusion_loss = compute_dcd_confusion_loss(outputs=dcd_out,labels=pair_labels)
            combined_loss = params.gamma * dcd_confusion_loss + clf_loss

            combined_loss.backward(retain_variables=True)
            opt_enc.step()
            opt_clf.step()
            opt_enc.zero_grad(); opt_clf.zero_grad(); opt_dcd.zero_grad()
